...
class Ui_Form(object):
    def setupUi(self, Form, typeList):
        ...
        self.result=Db.get_places_type(typeList)
    ...
    def openSelectedPlace(self):
        ...
        self.scity=None
        self.dcity=Db.cur.execute("SELECT C.CITY_NAME FROM CITY C,LOCALITY L,PLACESTOVISIT P "
                                  "WHERE C.CITY_NAME=L.CITY_NAME AND L.LOCALITY_ID=P.LOCALITY_ID "
                                  "AND P.PLACE_NAME='{}'".format(placeInfo[0][0])).fetchall()[0][0]
        self.form3=QtWidgets.QWidget()
        self.placeInformation=placeInformation()
        self.placeInformation.setupUi(self.form3,placeInfo[0],str(self.scity),str(self.dcity),1)
        self.form3.show()
